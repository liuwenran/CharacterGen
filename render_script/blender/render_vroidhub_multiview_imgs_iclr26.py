...
def setup_lighting(camera):
    ...
    # # Add ambient/fill light to reduce harsh shadows and prevent overexposure
    # ambient_data = bpy.data.lights.new(name="AmbientLight", type='SUN')
    # ambient_data.energy = 0.3  # Soft ambient light
    # ambient_data.color = (1.0, 1.0, 1.0)
    
    # ambient_object = bpy.data.objects.new(name="AmbientLight", object_data=ambient_data)
    # bpy.context.collection.objects.link(ambient_object)
    # ambient_object.location = (0, 0, 5)
    # ambient_object.rotation_euler = (0, 0, 0)
...
def render_views(folder, center=Vector((0, 0, 0)), model_height=1.0, model_width=1.0):
    ...
    # 设置渲染引擎为 Eevee（快速）或 Cycles（高质量）
    bpy.context.scene.render.engine = 'BLENDER_EEVEE'  # 使用 'CYCLES' 获得更真实的光照
    
    # 调整色彩管理，防止过曝
    bpy.context.scene.view_settings.view_transform = 'Standard'  # 或 'Filmic' 以获得更好的动态范围
    bpy.context.scene.view_settings.look = 'None'
    bpy.context.scene.view_settings.exposure = 0.0  # 降低曝光，可以调整为负值如 -0.5
