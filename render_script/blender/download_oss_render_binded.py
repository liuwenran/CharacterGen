...
if not args.all:
    ...
    finished_id_file = f'{info_dir}/finished_id_all.txt'
...
if not args.all:
    failed_id_file = f'{info_dir}/failed_id_{args.thread}.txt'
else:
    failed_id_file = f'{info_dir}/failed_id_all.txt'
...
for ind, line in enumerate(all_blend_id_lines):
    ...
    if os.path.exists(check_mp4):
        ...
        failed_id_file_handle.write(f"{blend_id}\n")
